...
real_T interp1_FTXwGFAn(const real_T varargin_1[38], const real_T varargin_2[38],
  real_T varargin_3)
{
  real_T Vq;
  real_T y[38];
  ...
  int32_T mid_i;
  int32_T exitg1;
  memcpy(&y[0], &varargin_2[0], 38U * (sizeof(real_T)));
  memcpy(&x[0], &varargin_1[0], 38U * (sizeof(real_T)));
  Vq = (rtNaN);
  low_i = 0;
  do {
    exitg1 = 0;
    if (low_i < 38) {
      ...
      if (((!rtIsNaN(varargin_3)) && (!(varargin_3 > x[37]))) && (!(varargin_3 <
            x[0]))) {
        low_i = 1;
        low_ip1 = 2;
        high_i = 38;
