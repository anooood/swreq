...
#include "inv_PGLFZ4cJ.h"

/* Function for MATLAB Function: '<S1>/NAV_Embedded_DoNavigationMain2Hz' */
void inv_PGLFZ4cJ(const real_T x[324], real_T y[324])
{
  int8_T p[18];
  real_T b_x[324];
  int32_T ipiv[18];
  ...
  for (b_k = 0; b_k < 18; b_k++) {
    ...
    for (j = b_k; (j + 1) < 19; j++) {
      i = (18 * pipk) + j;
      if (y[i] != 0.0) {
        for (kAcol = j + 1; (kAcol + 1) < 19; kAcol++) {
          y_tmp = (18 * pipk) + kAcol;
          y[y_tmp] -= y[i] * b_x[(18 * j) + kAcol];
        }
      }
    }
  }
